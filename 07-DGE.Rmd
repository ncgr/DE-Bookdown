...
<!-- Get the covid-19 pneumonia study into here at the end; also for the power chapter. -->

Make sure you are in a screen.

## R data import

Move into the DESeq2 directory.
...
<!-- This doesn't match what DESeq2 is reporting after the differential expression, presumable because of normalizations. Removing because it is confusing.

Count genes with totals > 0 across all samples.
...
Generate library size normalization factors.

> **_NOTE:_** DESeq2 will estimate these size factors if you don't but it is good to explicitly look at them.
...
![](./Figures/count_density_prenorm.png){width=100%}
...
![](./Figures/count_density_postnorm.png){width=100%}

Let's generate similar multidensity plots.

```{R, eval=FALSE}
pdf("multidensity_prenorm.pdf")
multidensity(counts(DESeqDataset, normalized=F)[idx.nz,],main="Density pre-normalized",xlab="mean counts", xlim=c(0,1000))
dev.off()
``` 

![](./Figures/multidensity_prenorm.png){width=100%}

```{R, eval=FALSE}
pdf("multidensity_postnorm.pdf")
multidensity(counts(DESeqSF, normalized=T)[idx.nz,],main="Density post-normalized",xlab="mean counts", xlim=c(0,1000))
dev.off()
``` 

![](./Figures/multidensity_postnorm.png){width=100%}
...
Finally, let's export a file of differentially expressed genes with counts appended to it.
...
In addition, let's save the normalized read counts for all the genes.

```{R, eval=FALSE}
write.table(as.data.frame(counts(dds, normalized=TRUE)), row.names=TRUE, quote=FALSE, sep="\t", file="normcounts.mouse.tsv"")
```
...
![](./Figures/heatmapSigs-1.png){width=100%}
...
![](./Figures/heatmapSigsMost_take1.pdf){width=100%}
...
![](./Figures/heatmapSigsMost-1.png){width=100%}
...
![](./Figures/PCA.allsamples2-1.png){width=100%}


<!-- With only 18 genes, ClueGO can't find anything even if you relax the p value. You get 50 genes if you call differential genes at p<=0.1 but that only gives one GO term even if you double the p value for the 2 sided test.-->
