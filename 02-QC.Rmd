...
We will run our own analysis on these publically-available data. The original study was published here:   
...
Make a new directory called reads and move into it.
...
We have a list of unique identifiers from 6 Sequencing Runs for sample sequences stored in the Sequence Read Archive (SRA) database managed by NCBI. We can use this simple list (in .txt format) to download the sequences that correspond to each of the 6 SRR identifiers in the list.  

Though we only have 6 samples, the sequencing **depth** is very high, making the download time ~ 1hr. 

**<span>Time will not allow for us to complete this command, so we will cancel the command after checking it works</span>.** 
...
done < ~/home/data/diffexpr/SRRlist.txt
```

Softlink the pre-downloaded data (after you cancel the command above):
```{bash, eval=FALSE}
ln -s /home/data/diffexpr/reads/*.gz .
```

## fastp Quality Control  

fastp (https://github.com/OpenGene/fastp) does both quality assessment and preprocessing. It will identify and trim adapters and remove low quality reads.
...
for file in SRR*.fastq.gz; do
  ...
  fastp -w 10 -h ${bn}.html -j ${bn}.json -i ${file} -o ${bn}.fastp.fastq.gz
done
```
...
You should be able to do this on your own but here is a hint for mac users (windows users should be able to download from the MobaXTerm GUI).

Mac users: From **your local terminal**:
```{bash, eval=FALSE}
scp -P 2403 agomez@inbre.ncgr.org:~/DGE_workshop/reads/SRR23869771.html
```
