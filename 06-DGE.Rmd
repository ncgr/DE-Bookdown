...
The sample columns end in .sorted.bam. Let's remove that suffix using the gsub (global substition) function.

```{R, eval=FALSE}
colnames(stress_from_control_sorted)=gsub(".sorted.bam","",colnames(test))
```
...
```{R, eval=FALSE}
stress_from_control_sorted_sub = subset(stress_from_control_signif, padj <= 0.05)
```

Finally, let's export a file with counts appended to it.

```{R, eval=FALSE}
write.table(merge(as.data.frame(stress_from_control_sorted_signif),as.data.frame(counts(dds,normalized=TRUE)), by="row.names"), row.names=FALSE, quote=FALSE, sep="\t", file="SfromC.tsv")
```
