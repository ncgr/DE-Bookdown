...
samtools flagstat SRR23869771.fastp.fastq.gz.sam
```
...
```{bash, eval=FALSE}
grep -v "^@" SRR23869771.fastp.fastq.gz.sam
```
...
```{bash, eval=FALSE}
grep -v "^@" SRR23869771.fastp.fastq.gz.sam | awk '{print $1}' | uniq | wc -l
```
...
```{bash, eval=FALSE}
cat SRR23869771.fastp.fastq.gz.sam | grep -v '^@' | awk '{ if ($3 != "*") print $0}' | awk '{print $1}' | uniq | wc -l
```
...
```{bash, eval=FALSE}
samtools view -f 0x100 -c SRR23869771.fastp.fastq.gz.sam 
```
...
```{bash, eval=FALSE}
samtools view -F 0x904 -c SRR23869771.fastp.fastq.gz.sam 
```
