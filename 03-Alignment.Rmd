...
**MAKE SURE YOUR ARE IN THE CORRECT DIRECTORY BEFORE YOU MAKE THIS DIRECTORY**</span>
...
Use wget command to download files using a link:
```{bash, eval=FALSE}
wget -4 https://ftp.ncbi.nlm.nih.gov/genomes/refseq/vertebrate_mammalian/Mus_musculus/latest_assembly_versions/GCF_000001635.27_GRCm39/GCF_000001635.27_GRCm39_genomic.fna.gz
```
...
```{bash, eval=FALSE}
wget -4 https://ftp.ncbi.nlm.nih.gov/genomes/refseq/vertebrate_mammalian/Mus_musculus/latest_assembly_versions/GCF_000001635.27_GRCm39/GCF_000001635.27_GRCm39_genomic.gff.gz
```
...
<span style="color:blue">Time will not allow for us to complete this command, so we will cancel the command after checking it works.</span>
```{bash, eval=FALSE}
...
This directory will be called "alignments" and <span style="color:blue">**MUST BE IN YOUR DGE_workshop DIRECTORY**</span>
...
Run hisat2 to align fastq files to the genome index. We'll use the fastq files that fastp filtered to remove low quality reads.

```{bash, eval=FALSE}
for file in *.fastp.fastq.gz; do
  base=`basename $file .fastp.fastq.gz`
  hisat2 -x ~/DGE_workshop/genome/GRCm39.index --threads 10 -U "$file" -S ~/DGE_workshop/alignments/"${base}.sam"
done
```
...
samtools flagstat SRR23869771.sam
```

The number in the first row is the count of total alignment records in the .sam file. Confirm this by counting the rows not including the header lines.

```{bash, eval=FALSE}
grep -v "^@" SRR23869771.sam | wc -l
```

Secondary alignments counts the alignments of reads that mapped to multiple locations on the genome. To omit these, count the number of unique read IDs in the file. This will provide the primary alignment counts.

```{bash, eval=FALSE}
grep -v "^@" SRR23869771.sam | awk '{print $1}' | uniq | wc -l
```

Check this with arithmatic from the flagstat output: 39304942 - 14794379
...
```{bash, eval=FALSE}
cat SRR23869771.sam | grep -v '^@' | awk '{ if ($3 != "*") print $0}' | awk '{print $1}' | uniq | wc -l
```
...
```{bash, eval=FALSE}
samtools view -f 0x100 -c SRR23869771.sam 
```
...
```{bash, eval=FALSE}
samtools view -F 0x904 -c SRR23869771.sam 
```
...
```{bash, eval=FALSE}
for f in *.fastp.sam.sorted; do
    mv "$f" "${f/.fastp.sam.sorted/.sorted.bam}"
done
```
...
Review the steps of the workflow up until now.
