...
<!-- Make sure people save their R workspace if this splits over multiple sessions; or I can copy my R history files-->
...
<!-- is the p for pearson? or pairwise complete obs?); should this be the wgcna correlation? -->
...
age and days_from_first_intubation are already numeric so we can leave them as is. But days_from_first_intubation has one entry that is Inf (infinity), presumably meaning that the person was never intubated. This messes up downstream analyses because some of them don't consider Inf as a numeric value. Plus the one patient that wasn't intubated is in a category all their own. Let's replace that Inf value with NA.

```{R, eval=FALSE}
heatmap.data$days_from_first_intubation[heatmap.data$days_from_first_intubation>=Inf]=NA
```
...
```{R, eval=FALSE}
heatmap.data$days_from_first_intubation[heatmap.data$days_from_first_intubation<=10]=0
heatmap.data$days_from_first_intubation[heatmap.data$days_from_first_intubation>10]=1
```
...
ENSG00000165949 is IFI27 (interferon alpha inducible protein 27). It is induced by interferon and regulates transcription. It is known to be involved in defense response and apoptosis pathways.
